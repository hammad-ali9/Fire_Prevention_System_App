...
const CONNECTOR_SHARED_SECRET = defineSecret("CONNECTOR_SHARED_SECRET");
const DM_API_KEY = defineSecret("DM_API_KEY");

// Device Manager (OEM Server) API base — EU region.
const DM_API_BASE = "https://api-eu.oemserver.com";

// ── Normalized telemetry doc the Flutter app reads ─────────────────────────
interface DeviceDoc {
  serial: string;
...
function fieldByType(rec: any, ftype: number): any {
  const fields: any[] = rec?.Fields ?? rec?.fields ?? [];
  for (const f of fields) {
    if ((f?.FType ?? f?.ftype) === ftype) return f;
  }
  return {};
}

/** First field in `Fields` that actually carries a given key (FType-agnostic). */
function fieldWith(rec: any, key: string): any {
  const fields: any[] = rec?.Fields ?? rec?.fields ?? [];
  for (const f of fields) {
    if (f && f[key] !== undefined && f[key] !== null) return f;
  }
  return {};
}
 ...
 */
function mapRecord(serial: string, rec: any, assetName: string | null): DeviceDoc {
  ...
  const ad: Record<string, unknown> =
    analogueField.AnalogueData ?? analogueField.analogueData ?? {};
  ...
  const lastSeen = toTimestamp(
    rec.DateUTC ?? rec.dateUtc ?? gps.GpsUTC ?? gps.gpsUTC,
  );
  ...
  const sprinklerActive =
    dout === null ? null : ((dout >> VALVE_OUTPUT_BIT) & 0x1) === 0x1;
...
function toTimestamp(v: unknown): FirebaseFirestore.Timestamp | null {
  if (v === null || v === undefined) return null;
...
export const dmConnectorIngest = onRequest(
  {secrets: [CONNECTOR_SHARED_SECRET], region: "europe-west1", cors: false},
  async (req, res) => {
    if (req.method !== "POST") {
    ...
    // NOTE: if you configure the connector with the Cryptographic Key Pair
    //    instead of a shared secret, replace this with signature verification.
    const provided =
      req.get("X-Connector-Secret") ??
    ...
    try {
      ...
      const serial =
        `${body.SerNo ?? body.serNo ?? body.SN ?? body.serial ?? ""}`;
      const assetName = body.assetName ?? body.AssetName ?? null;
      const records: any[] = body.Records ?? body.records ?? [];
...
function recordMs(rec: any): number {
...
export const dmSetOutput = onCall(
  {secrets: [DM_API_KEY], region: "europe-west1"},
  async (request) => {
    if (!request.auth) {
    ...
    if (!serial) {
      throw new HttpsError("invalid-argument", "serial is required.");
    }

    const ok = await toggleDeviceOutput(serial, active, DM_API_KEY.value());
    if (!ok) {
      throw new HttpsError("internal", "Device Manager rejected the command.");
    }
 ...
 */
async function toggleDeviceOutput(
  serial: string,
  active: boolean,
  apiKey: string,
): Promise<boolean> {
  ...
  const url =
    `${DM_API_BASE}/v1/AsyncMessaging/Send` +
  ...
  if (!ok) {
    logger.error(
      `toggleDeviceOutput: ${serial} → HTTP ${resp.status} ${await resp.text()}`,
    );
    return false;
  }
  logger.info(
    `toggleDeviceOutput: ${serial} valve ${active ? "ON" : "OFF"} queued`,
  );
  return true;
}
